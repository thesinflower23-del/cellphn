...
function switchView(view) {
  ...
  switch (view) {
    ...
    case 'gallery':
      ...
      break;
    case 'upliftRequests':
      document.getElementById('upliftRequestsView').style.display = 'block';
      loadUpliftRequestsView();
      break;
    case 'account':
      if (accountView) {
...
async function loadUpliftRequestsView() {
  // ensure admin
  if (!(await requireAdmin())) return;

  const container = document.getElementById('upliftRequestsTable');
  if (!container) return;

  container.innerHTML = '<p class="empty-state">Loading uplift requests...</p>';

  try {
    // Prefer firebase helper if available
    let requests = [];
    if (typeof getUpliftRequests === 'function') {
      requests = await getUpliftRequests();
    } else {
      requests = JSON.parse(localStorage.getItem('upliftRequests') || '[]');
    }

    if (!Array.isArray(requests) || requests.length === 0) {
      container.innerHTML = '<p class="empty-state">No uplift requests found.</p>';
      return;
    }
    ...
    container.innerHTML = `
      ...
      const userLabel = r.email ? `${escapeHtml(r.email)}` : (r.userId ? escapeHtml(r.userId) : 'Anonymous');
      const status = escapeHtml(r.status || 'pending');
      const proofCell = r.proofUrl ? `<a href="${escapeHtml(r.proofUrl)}" target="_blank">View</a>` : (r.proof ? `<a href="#" class="view-proof-inline" data-id="${escapeHtml(r.id)}">View</a>` : '—');
      const shortNote = r.note ? escapeHtml(r.note).slice(0, 120) : '';
      return `
              <tr data-id="${escapeHtml(r.id)}">
                <td>${when}</td>
                <td>${userLabel}</td>
                <td style="max-width:320px; white-space:pre-wrap;">${shortNote}</td>
                <td>${proofCell}</td>
                <td>${status}</td>
                <td>
                  ${r.status === 'approved' ? '<span style="color:var(--success-600); font-weight:700;">Approved</span>' : `<button class="btn btn-success btn-sm" data-approve="${escapeHtml(r.id)}">Approve</button>`}
                  ${r.status === 'denied' ? '<span style="color:var(--danger); font-weight:700;">Denied</span>' : `<button class="btn btn-danger btn-sm" data-deny="${escapeHtml(r.id)}">Deny</button>`}
                </td>
              </tr>
            `;
    }).join('')}
    ...
    container.querySelectorAll('[data-approve]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        await handleApproveUplift(id);
        await loadUpliftRequestsView();
      });
    });
    container.querySelectorAll('[data-deny]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const id = btn.dataset.deny;
        await handleDenyUplift(id);
        await loadUpliftRequestsView();
      });
    });

    // Inline proof viewing for data URL proofs saved locally
    container.querySelectorAll('.view-proof-inline').forEach(a => {
      a.addEventListener('click', (e) => {
        ...
        if (req.proof) {
          showModal(`<h3>Proof</h3><img src="${escapeHtml(req.proof)}" style="max-width:100%; height:auto;" />`);
        } else {
          customAlert.info('No proof available');
        }
      });
    });

  } catch (err) {
    console.error('Failed loading uplift requests', err);
    container.innerHTML = '<p class="empty-state">Error loading requests. Check console.</p>';
  }
}

async function handleApproveUplift(id) {
  try {
    const confirmed = await customAlert.confirm('Approve uplift', 'Approve this uplift request and lift ban for the user if applicable?');
    if (!confirmed) return;
    // fetch request
    let requests = [];
    if (typeof getUpliftRequests === 'function') requests = await getUpliftRequests();
    else requests = JSON.parse(localStorage.getItem('upliftRequests') || '[]');
    const req = requests.find(r => r.id === id);
    if (!req) {
      customAlert.error('Request not found');
      return;
    }

    // Update status
    const updates = { status: 'approved', reviewedAt: Date.now(), reviewedBy: (await getCurrentUser())?.id || 'admin' };
    if (typeof updateUpliftRequest === 'function') {
      await updateUpliftRequest(id, updates);
    } else {
      // persist to localStorage
      const existing = JSON.parse(localStorage.getItem('upliftRequests') || '[]');
      const idx = existing.findIndex(x => x.id === id);
      if (idx !== -1) { existing[idx] = { ...existing[idx], ...updates }; localStorage.setItem('upliftRequests', JSON.stringify(existing)); }
    }

    // If request references a userId, lift ban
    if (req.userId) {
      try {
        await liftCustomerBan(req.userId, { reason: 'Admin approved uplift request' });
        customAlert.success('Uplift approved and ban lifted (if any).');
      } catch (e) {
        console.warn('liftCustomerBan failed', e);
        customAlert.success('Uplift approved. Please manually lift user ban.');
      }
    } else {
      customAlert.success('Uplift approved.');
    }
  } catch (e) {
    console.error('Approve uplift failed', e);
    customAlert.error('Could not approve request. See console.');
  }
}

async function handleDenyUplift(id) {
  try {
    const reason = prompt('Reason for denying this uplift request (optional):', 'Insufficient proof');
    if (reason === null) return; // cancelled
    const updates = { status: 'denied', reviewedAt: Date.now(), reviewedBy: (await getCurrentUser())?.id || 'admin', reviewNote: reason };
    if (typeof updateUpliftRequest === 'function') {
      await updateUpliftRequest(id, updates);
    } else {
      const existing = JSON.parse(localStorage.getItem('upliftRequests') || '[]');
    ...
    customAlert.info('Uplift request denied.');
  } catch (e) {
    console.error('Deny uplift failed', e);
    customAlert.error('Could not deny request. See console.');
  }
}
