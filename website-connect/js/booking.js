...
async function renderCalendarTimePicker() {
  ...
  // Precompute per-day stats (bookings, absences, capacity) for the visible month
  let monthDataset = {};
  try {
    const bookings = Array.isArray(await getBookings()) ? await getBookings() : [];
    const absences = Array.isArray(await getStaffAbsences()) ? await getStaffAbsences() : [];
    const groomers = Array.isArray(await getGroomers()) ? await getGroomers() : [];

    for (let d = new Date(firstDayOfMonth); d <= lastDayOfMonth; d.setDate(d.getDate() + 1)) {
      const iso = toLocalISO(d);
      const dayBookings = bookings.filter(b => b.date === iso);
      const dayAbsences = absences.filter(a => a.date === iso && !['rejected', 'cancelledByStaff'].includes(a.status));
      const blackout = typeof getCalendarBlackout === 'function' ? getCalendarBlackout(iso) : null;
      if (blackout) {
        monthDataset[iso] = {
        ...
        const totalGroomers = (groomers && groomers.length) ? groomers.length : DEFAULT_GROOMERS.length;
    ...
    console.warn('Failed to compute month dataset for calendar:', e);
    monthDataset = {};
  }
  ...
  for (let day = 1; day <= lastDayOfMonth.getDate(); day++) {
    ...
    days.push({ day, iso, stats: monthDataset[iso] || null });
  }
  ...
  container.innerHTML = `
    ...
    const { bookings = 0, absences = 0, capacityStatus = 'green', remaining = 0, capacity = 0, blackout: blackoutInfo = null } = stats || {};
    const hasEvents = bookings > 0 || absences > 0;
    const isPast = isPastDate(day.iso);
    const isClosed = capacityStatus === 'blackout' || !!blackoutInfo;
    const isSelected = selectedDate === day.iso;
    const slotsText = isClosed ? 'Closed' : isPast ? 'Not available' : (remaining > 0 ? `${remaining} slots left` : 'Fully booked');
    const bookedText = bookings > 0 ? `${bookings} booked` : '';
    return `
            <div class="calendar-cell day ${capacityStatus ? 'status-' + capacityStatus : ''} ${hasEvents ? 'has-events' : ''} ${isPast ? 'past' : ''} ${isClosed ? 'blackout' : ''} ${isSelected ? 'selected' : ''}"
                 data-date="${day.iso}"
              ...
              ${capacityStatus !== 'blackout' ? `<div class="capacity-pill" style="font-size: 0.75rem; margin-top: 0.25rem; font-weight: 600;">${slotsText}</div>` : ''}
